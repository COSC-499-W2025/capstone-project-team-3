...
def init_db():
    ...
    _ensure_user_preferences_linkden_column(cursor)
...
def _ensure_user_preferences_linkden_column(cursor: sqlite3.Cursor) -> None:
    """Ensure USER_PREFERENCES has linkden column on existing DBs."""
    cursor.execute("PRAGMA table_info(USER_PREFERENCES)")
    existing_columns = {row[1] for row in cursor.fetchall()}
    if "linkden" not in existing_columns:
        cursor.execute("ALTER TABLE USER_PREFERENCES ADD COLUMN linkden TEXT")
