...
class TestInstitutionSearch:
    """Test suite for institution search endpoints."""
    
    @patch("urllib.request.urlopen")
    def test_search_institutions_full_success(self, mock_urlopen):
        """Test full search with program details."""
        # Mock the API response at urllib level
        ...
        response = client.get("/api/institutions/search?q=University&limit=10")
        ...
        assert "programs" in data["institutions"][0]

    @patch("urllib.request.urlopen")
    def test_search_institutions_simple_mode(self, mock_urlopen):
        """Test simple search that returns only names."""
        mock_response = MagicMock()
        mock_response.read.return_value = json.dumps({
            "success": True,
            "result": {
                "records": [
                    {"institution_name_e": "University of Toronto"},
                    {"institution_name_e": "University of Waterloo"},
                    {"institution_name_e": "University of British Columbia"}
                ]
            }
        }).encode()
        ...
        response = client.get("/api/institutions/search?q=University&limit=20&simple=True")
        ...
        if data["institutions"] and isinstance(data["institutions"][0], dict):

            institution_names = [inst["name"] if isinstance(inst, dict) else inst 
                                for inst in data["institutions"]]
            assert set(institution_names) == {
            ...
            assert all(isinstance(inst, str) for inst in data["institutions"])
            assert set(data["institutions"]) == {
                "University of Toronto",
                "University of Waterloo",
                "University of British Columbia"
            }
    ...
    @patch("urllib.request.urlopen")
    def test_search_institutions_no_query(self, mock_urlopen):
        """Test search without query parameter returns results."""
        mock_response = MagicMock()
        mock_response.read.return_value = json.dumps({
            "success": True,
            "result": {
                "records": [
                    {
                        "institution_name_e": "Conestoga College",
                        "program_of_study_e": "Applied Health Information Science",
                        "academic_level_area_of_study_e": "Applied Degree",
                        "program_type_e": "CO-OP"
                    }
                ]
            }
        ...
        response = client.get("/api/institutions/search?limit=50")
    ...
    @patch("urllib.request.urlopen")
    def test_search_institutions_empty_results(self, mock_urlopen):
        ...
        response = client.get("/api/institutions/search?q=NonexistentUniversity&limit=10")
    ...
    def test_search_institutions_api_error(self):
        """Test search with invalid limit parameter."""
        response = client.get("/api/institutions/search?q=University&limit=500")
